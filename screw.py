...
class FHscrew(object):
    ...
    def hole(self):
        ...
        cntr_sink=Cylinder(cntr_sink_ht,rad=self.cs_dia/2,r2=0.01,fs=.1)

        top = Cylinder(h=1,rad=self.cs_dia/2,fs=0.1)
        top.translate ([0,0,-1])

        shank = Cylinder(h=self.length+1,rad=self.thd/2,fs=0.1)
